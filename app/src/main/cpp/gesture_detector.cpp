...
/**
 * Convert data from camera to RGB Mat
 */
Mat convertByteArrayToRGBMat(jbyte *bytes, int imageWidth, int imagHeight) {
    // data from android camera in YUV format
    Mat yuvMat(imagHeight + imagHeight / 2, imageWidth, CV_8UC1, (void *) bytes);
...
extern "C"
void Java_sh_kostya_handgesturedetector_GestureDetectorNative_init(JNIEnv *env, jobject /* this */) {
    bg.set("nmixtures", 3);
...
extern "C"
jint Java_sh_kostya_handgesturedetector_GestureDetectorNative_recognizeGesture(JNIEnv *env, jobject /* this */,
                                                         jbyteArray cameraData,
                                                         jint cameraWidth,
                                                         jint cameraHeight) {
    jbyte *rawCameraData = env->GetByteArrayElements(cameraData, 0);
    frame = convertByteArrayToRGBMat(rawCameraData, cameraWidth, cameraHeight);
    ...
    for (int i = 0; i < contours.size(); i++) {
        ...
        if (contourArea(contours[i]) >= 5000) {
            ...
            if (hullsI[0].size() > 0) {
                ...
                for (int j = 0; j < 4; j++)
                    line(frame, rect_points[j], rect_points[(j + 1) % 4], Scalar(255, 0, 0), 1,
                         8);
                ...
                if (defects.size() >= 3) {
                            ...
                            if (min(Xdist, Ydist) / max(Xdist, Ydist) <= 0.8) {
                                if ((Xdist >= 0.1 * radius && Xdist <= 1.3 * radius &&
                                    ...
                                    (Ydist >= 0.1 * radius && Ydist <= 1.3 * radius &&
                                     Xdist > Ydist))
                                    line(frame, ptEnd, ptFar, Scalar(0, 255, 0),
                                         1), no_of_fingers++;
                            }
                    ...
                    syslog(LOG_DEBUG, "number of fingers = %d", no_of_fingers);
                    if (no_of_fingers > 0) {
                        return no_of_fingers;
                    }
                }
            }
    ...
    if (backgroundFrame > 0)
        putText(frame, "Recording Background", cvPoint(30, 30), FONT_HERSHEY_COMPLEX_SMALL, 0.8,
